...
check() {
  ...
  local got; got=$(curl -s -o "$BODY" -w '%{http_code}' -H "X-Correlation-Id: $CID" "${AUTH[@]}" "$@")
  [[ "$got" == "$want" ]] && pass "$desc -> $got" || { fail "$desc -> $got (expected $want): $(head -c 200 "$BODY")"; }
}
...
grep -q '"status":"Done"' "$BODY" && pass "     status is Done" || fail "     status not updated"
...
check "DELETE /api/tasks/{id} (permanent)" 204 -X DELETE "$API/api/tasks/$ID?permanent=true"
